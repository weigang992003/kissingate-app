...
var wsoLogger = new Logger('web-socket-polling-monitor');
...
function createOffer(order1, order2) {
    ...
    var order1_gets_balance = tls.getBalance(ausj.getIssuer(order1.TakerGets), aujs.getCurrency(order1.TakerGets));
    var order2_pays_balance = tls.getBalance(aujs.getIssuer(order2.TakerPays), aujs.getCurrency(order2.TakerPays));
    var order2_gets_balance = tls.getBalance(ausj.getIssuer(order2.TakerGets), aujs.getCurrency(order2.TakerGets));
    ...
    if (min_taker_pays.compareTo(order1_taker_pays.product_human(times)) == 1) {
        order1_taker_gets = min_taker_gets;
        ...
        times = math.round(times - 0, 6);
        order2_taker_pays = min_taker_gets;
        ...
        times = math.round(times - 0, 6);
        order1_taker_pays = min_taker_pays;
        ...
        times = math.round(times - 0, 6);
        order2_taker_gets = min_taker_pays;
        order2_taker_pays = order2_taker_gets.product_human(times);
    }
